...
const slotClassArr = [ ".s1", ".s2", ".s3", ".s4", ".s5" ];
...
function magicAct (num1, num2, num3, num4, num5) {
    //reset icons
    disappear();
    for (let i = 0;i < iconIdsArr.length;i++) {
        //check slot one
        ...
        //check slot four
        if (num4 == i) {
            $(".s4").find(`${ iconIdsArr[ num4 ] }`)
                .css("display", "initial");
        }
        //check slot five
        if (num5 == i) {
            $(".s5").find(`${ iconIdsArr[ num5 ] }`)
                .css("display", "initial");
        }
    }
}
//checks to see if the player has won
function winCondition (num1, num2, num3, num4, num5, spinCount) {
    if (num1 == num2 && num2 == num3 && num3 == num4 && num4 == num5) {
        ...
        $("#reset-btn").classList.add("flex", "justify-center", "items-center");
...
$(document).ready(function () {
    ...
    $("#spin-btn").click(function () {
        ...
        document.getElementById("spin-btn").classList.remove("bg-green-500", "hover:bg-green-600");
        document.getElementById("spin-btn").classList.add("bg-gray-500", "cursor-no-drop");
        setTimeout(() => {
            ...
            //CASE -- We Lost:
                ...
                console.log(`DEBUG: (${ dice[ 0 ] }, ${ dice[ 1 ] }, ${ dice[ 2 ] }, ${ dice[ 3 ] }, ${ dice[ 4 ] })`); //debug

                //display the appropriate icons based on the random indices
                magicAct(dice[ 0 ], dice[ 1 ], dice[ 2 ], dice[ 3 ], dice[ 4 ]);
                // css animation for slot icons

                //check if we've won
                winCondition(dice[ 0 ], dice[ 1 ], dice[ 2 ], dice[ 3 ], dice[ 4 ], spinCount);
            }
            document.getElementById("spin-btn").disabled = false;
            document.getElementById("spin-btn").classList.add("bg-green-500", "hover:bg-green-600");
            document.getElementById("spin-btn").classList.remove("bg-gray-500", "cursor-no-drop");
        }, 1800);
        //refresh stats
        refreshStats();
    });
